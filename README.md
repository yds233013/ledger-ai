...
Upload a bank statement — CSV, or a PDF read from its text layer — or photograph
a receipt, and Ledger AI normalizes, de-duplicates and categorizes the
transactions, flags duplicates and unusual charges with the statistics that
produced them, and answers questions in plain English — showing the plan it
built, the SQL aggregate it ran, and the rows it ran over, at every step.

> **Invite-only private beta.** Ledger AI is a personal finance workspace open
> only to invited accounts; there is no public sign-up. Every figure in this
> repository and in every screenshot is **synthetic**, generated by a seeded
> PRNG. Ledger AI does not connect to any real financial institution, holds no
> real financial data, and does not provide financial, investment or tax advice.
...
**Two authentication paths, chosen per audience.** The public demo runs on
Auth.js with HS256, because anyone cloning this repository would otherwise need
a third-party account before the demo would start, and the build must run
offline. The invite-only private beta runs on Clerk with RS256/JWKS, which is
the correct choice once real accounts and a hosted identity provider exist.
Invitations are enforced twice over: the Clerk instance is restricted, so it
refuses to create an account for an uninvited address, and the API independently
requires a local invitation bound to that address. See
